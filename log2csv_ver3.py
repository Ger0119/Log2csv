    ...
    with open("temp.csv") as f:
        for line in f:
            ...
            if ind == 0:
                ...
                r_lst = Pro.merge_lst(r_lst,temp)
                ind = 1
            else:
                ind = 0

    r_lst = ["Wno", "X", "Y", "P/F", "DUT", "FailTest", "BIN", "Alarm"] + r_lst
    Pro.fix_temp_file(r_lst)
    base = pd.read_csv("temp_data.csv",index_col=0,header=None,low_memory=False,dtype=str)
    base = base.iloc[:,:-1]
    base.columns = r_lst
    os.remove("temp.csv")
...
class Solution(object):
    ...
    def fix_temp_file(self,lst):
        out = open("temp_data.csv","w+")
        dic= {}
        flag = 0
        with open("temp.csv") as f:
            for line in f:
                if not line:
                    break
                if flag == 0:
                    keys = ["Wno", "X", "Y", "P/F", "DUT", "FailTest", "BIN", "Alarm"] + self.fix_Index(line.strip().split(",")[8:])
                    flag = 1
                else:
                    values = line.strip().split(",")
                    for i in range(len(keys)):
                        dic[keys[i]] = values[i]
                    out.write(f'{dic["Wno"]}.{dic["X"]}.{dic["Y"]},')
                    for x in lst:
                        if x in dic:
                            out.write(f"{dic[x]},")
                        else:
...
修正歴 :  2022/01/19 tempファイル処理Func追加　
"""
